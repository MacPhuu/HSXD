...
export default {
  ...
  methods: {
    ...
    async getFile() {
      ...
      let uriPFD = encodeURI(this.pathPdf)
      let uriFile = encodeURI(this.file.path)
      console.log('hihi',uriFile)
      if (this.pathPdf) {
        // this.urlPDF = `${config.api.download}/${this.pathPdf.replace('#', '%23')}`
        this.urlPDF = `${config.api.download}/${uriPFD}`
      } else {
        // this.urlPDF = `${config.api.download}/${this.file.path.replace('#', '%23')}`
        this.urlPDF = `${config.api.download}/${uriFile}`
      }
      this.fileSignData = {
    ...
    handleDownloadFile() {
      let uri
      if (this.pathPdf) {
        uri = `${config.api.download}/${this.pathPdf}`
      } else {
        uri = `${config.api.download}/${this.file.path}`
      }
      let uriDL = uri.replace('#', '%23')
      this.downloadURI(uriDL, this.file.name)
    }
  }
}
</script>
