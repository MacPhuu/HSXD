  ...
  <div v-loading="loadingPage" class="file-page">
    <div class="header-action">
      <div v-if="selectedList.length > 1">
          ...
          <el-dropdown-menu slot="dropdown-menu">
            <el-dropdown-item>
              <button-icon
                ...
                icon="Download"
                @click="handleDownloadMultiFile"
              >Tải xuống</button-icon>
            </el-dropdown-item>
            <el-dropdown-item>
              <el-popconfirm
                ...
                @confirm="handleDeleteMultiFile"
              >
                <button-icon slot="reference" icon="Trash">Xóa</button-icon>
              </el-popconfirm>
            </el-dropdown-item>
            <el-dropdown-item>
              <button-icon v-if="currentFile.default !== 1" icon="FolderMove" @click="handleMoveMulti">Chuyển tới</button-icon>
            </el-dropdown-item>
          </el-dropdown-menu>
        </button-dropdown>
        <!--        <button-icon-->
        <!--          v-loading="loadingDownload"-->
        <!--          icon="Download"-->
        <!--          @click="handleDeleteMultiFile"-->
        <!--        >Xóa</button-icon>-->
        <!--        <button-icon icon="FileCopy" @click="handleCopyMultiFile">Tạo bản sao</button-icon>-->
      </div>
      <div v-else-if="!selectedList.length">
        <button-dropdown icon="_Add">
          <el-dropdown-menu slot="dropdown-menu">
            <el-dropdown-item @click.native="addFolder">
              <svg-icon icon-class="FolderAdd" style="margin-right: 5px; font-size: 16px" />
              Thư mục mới
            </el-dropdown-item>
            <el-dropdown-item divided>
              ...
              >
                <span>
                  <svg-icon icon-class="FileUpload" style="margin-right: 5px; font-size: 16px" />
                  Tải lên tệp tin
                </span>
              </el-upload>
            </el-dropdown-item>
            <el-dropdown-item @click.native="handleUploadFolder">
              <svg-icon icon-class="FolderUpload" style="margin-right: 5px; font-size: 16px" />
              Tải lên thư mục
            </el-dropdown-item>
            <el-dropdown-item divided @click.native="addLink">
              <svg-icon icon-class="Link" style="margin-right: 5px; font-size: 16px" />
              Thêm link mới
            </el-dropdown-item>
            <!--            <el-dropdown-item divided @click.native="showCategory(categoryTypeMap.MATERIAL)">-->
            <!--              <svg-icon icon-class="List" style="margin-right: 5px; font-size: 16px" />-->
            <!--              Thêm danh mục vật liệu-->
            <!--            </el-dropdown-item>-->
            <!--            <el-dropdown-item @click.native="showCategory(categoryTypeMap.TASK)">-->
            <!--              <svg-icon icon-class="List" style="margin-right: 5px; font-size: 16px" />-->
            <!--              Thêm danh mục công việc-->
            <!--            </el-dropdown-item>-->
            <!--            <el-dropdown-item @click.native="showCategory(categoryTypeMap.SOURCE_MATERIAL)">-->
            <!--              <svg-icon icon-class="List" style="margin-right: 5px; font-size: 16px" />-->
            <!--              Thêm danh mục nguồn gốc vật liệu-->
            <!--            </el-dropdown-item>-->
          </el-dropdown-menu>
          <a class="add">Thêm mới</a>
        </button-dropdown>
        <!--        <button-icon v-if="categoryMaterial.length" icon="List" @click="showCategory(categoryTypeMap.MATERIAL)">Danh mục vật liệu</button-icon>-->
        <!--        <button-icon v-if="categoryTask.length" icon="List" @click="showCategory(categoryTypeMap.TASK)">Danh mục công việc</button-icon>-->
        <!--        <button-icon v-if="categorySourceMaterial.length" icon="List" @click="showCategory(categoryTypeMap.SOURCE_MATERIAL)">Nguồn gốc vật liệu</button-icon>-->
        <!--        <button-icon v-if="isCategory" icon="Reject" @click="isCategory = false">Ẩn danh mục</button-icon>-->
      </div>
      <div v-if="currentFile._id">
        <template v-if="currentFile.metaType === metaType.LINK">
          <button-icon icon="Edit" @click="editLink(currentFile)">Chỉnh sửa</button-icon>
          <el-popconfirm
            title="Bạn có chắc chắn xóa?"
            cancel-button-text="Hủy"
            confirm-button-text="Đồng ý"
        ...
        <template v-else>
          <button-dropdown> Options
            <el-dropdown-menu slot="dropdown-menu">
              <el-dropdown-item>
                <button-icon v-if="currentFile.metaType === metaType.FILE" v-loading="loadingDownload" icon="Download" @click="handleDownloadFile(currentFile)">Tải xuống</button-icon>
              </el-dropdown-item>
              <el-dropdown-item>
                <button-icon v-if="currentFile.metaType !== metaType.FILE" v-loading="loadingDownload" icon="Download" @click="handleDownloadMultiFile">Tải xuống</button-icon>
              </el-dropdown-item>
              <el-dropdown-item>
                <button-icon icon="Tag" @click="handleAssignTag(currentFile)">Gắn tag</button-icon>
              </el-dropdown-item>
              <el-dropdown-item>
                <button-icon v-if="currentFile.default !== 1" icon="FolderMove" @click="handleMove">Chuyển tới</button-icon>
              </el-dropdown-item>
              <el-dropdown-item>
                <el-popconfirm
                  title="Bạn có chắc chắn xóa?"
              ...
              <el-dropdown-item>
                <button-icon
                  v-if="currentFile.metaType === metaType.FILE && !currentFile.signState"
                  icon="Key"
                  @click="handleDigitizing(currentFile)"
                >
                  Số hóa
                </button-icon>
              </el-dropdown-item>
              ...
              <el-dropdown-item>
                <button-icon icon="Edit" @click="handleRename">Đổi tên</button-icon>
              </el-dropdown-item>
            </el-dropdown-menu>
          </button-dropdown>
        </template>
      ...
      <span class="right-action-btn">
        <template v-if="selectedList.length">
          <span class="selected-label">Đã chọn {{ selectedList.length }}</span>
          <button-icon icon="Close" @click="handleCancelSelect" />
        </template>
        <template v-else>
    ...
    <div v-else class="app-container">
      <el-row class="heading-container" type="flex" justify="space-between">
        <el-col :span="12">
          ...
          <h3 v-else style="margin: 0; padding: 0 5px; font-weight: 600; line-height: 32px">Quản lý tài liệu</h3>
        </el-col>
        <el-col class="finder" :span="8" style="text-align: right;display: flex;">
          <el-select
          ...
          >
            ...
            <el-option-group label="Tags">
              <el-option
                ...
                :value="tag._id"
              >
                <el-tag style="height: 28px" size="small" type="info">{{ tag.name }}</el-tag>
              </el-option>
            </el-option-group>
          </el-select>
          <el-input
          ...
          >
            ...
            >
              ...
              <el-row justify="end" type="flex">
                ...
                <el-button size="small" type="primary" :loading="loadingFilter" @click="handleConfirmFilter">Áp dụng
                </el-button>
              </el-row>
            </el-form>
            <el-button
      ...
      <div v-if="!loadingPage && tableData.length" class="table-wrap">
        <div v-if="viewType === config.viewType.LIST" class="table">
          <el-table
            ref="tableData"
            fit
            :data="pagedTableData"
          ...
          >
            <el-table-column
              ...
              width="50"
            />
            <el-table-column label="Tên" min-width="450" fixed class-name="selectable-cell">
              <template slot-scope="scope">
                ...
                >
                  <span style="cursor: pointer" @click="gotoDetail(scope.row)">
                    <svg-icon
                      ...
                      class="icon-svg"
                      icon-class="FolderFixed"
                      style="font-size:18px; margin-right: 5px"
                    />
                    <svg-icon v-else class="icon-svg" icon-class="FolderFlex" style="font-size:18px; margin-right: 5px" />
                ...
                >
                  <span style="cursor: pointer" @click="gotoDetail(scope.row)">
                    <svg-icon
                      v-if="fileType(scope.row.name) === 'docx' || fileType(scope.row.name) === 'doc'"
                      class="icon-svg"
                      icon-class="doc"
                    />
                    <svg-icon v-else-if="fileType(scope.row.name) === 'pdf'" class="icon-svg" icon-class="pdf" />
                    <svg-icon
                      v-else-if="fileType(scope.row.name) === 'xls' || fileType(scope.row.name) === 'xlsx'"
                      class="icon-svg"
                      icon-class="xls"
                    />
            ...
            <el-table-column prop="phaseState" label="Cập nhật" width="180">
              <template slot-scope="scope">
              ...
              <template slot-scope="scope">
                ...
                >
                  ...
                  <div v-for="tag in scope.row.tags" :key="tag._id">
                    <el-tag type="info" size="small" style="margin-bottom: 10px; height: 28px">
                      {{ tag.name }}
                    </el-tag>
                  </div>
                  <span slot="reference">
            ...
            <el-table-column prop="size" label="Kích thước" width="150">
              <template slot-scope="scope">
                <span
                  v-if="scope.row.metaType === metaType.FILE || scope.row.metaType === metaType.FOLDER"
                >{{ scope.row.size >= 0 ? getFileSize(scope.row.size) : "-" }}</span>
                <span v-else>-</span>
              </template>
            </el-table-column>
            <el-table-column prop="signState" label="Trạng thái số hóa" width="220" fixed="right">
              <template slot-scope="scope">
                <span v-if="scope.row.metaType === metaType.FILE">
                  <template v-if="scope.row.digital && scope.row.digital.sr">
                    <span class="icon-status" :style="{ background: stateConfig[scope.row.digital.sr.state].color }" />
                    <span
                      class="title-status"
                      :style="{ color: stateConfig[scope.row.digital.sr.state].color }"
                    >{{ stateConfig[scope.row.digital.sr.state].title }}</span>
                  </template>
                  <template v-else>
                    <span style="color: #97a8be; font-size: .9rem">Chưa số hóa</span>
                  </template>
                </span>
              </template>
            </el-table-column>
            <el-table-column class-name="right-action-table" prop="phaseSigned" align="center" width="120" fixed="right">
              <template v-if="scope.row.showButton" slot-scope="scope">
                <span
                  v-if="scope.row.metaType===metaType.FOLDER"
                  class="btn-table"
                ...
                <span
                  v-if="scope.row.metaType===metaType.FILE && !scope.row.signState"
                  class="btn-table"
                ...
                <span
                  v-if="scope.row.metaType===metaType.LINK"
                  class="btn-table"
        ...
        <div v-else-if="viewType === config.viewType.GRID" class="grid-box-container">
          <div class="grid-box">
            ...
            >
              <el-checkbox
                class="checkbox-item"
                @change="value => onSelectFile(item, value)"
              ...
              <svg-icon
                v-else-if="fileType(item.name) === 'docx' || fileType(item.name) === 'doc'"
    ...
    <el-dialog
      :title="formTypeFolder === 'create' ? 'Thêm thư mục' : 'Cập nhật thư mục'"
    ...
    >
      <el-form ref="formFolder" :model="form" label-position="top" :rules="ruleFormFolder">
        <el-form-item label="Tên Thư Mục" prop="name">
          <el-input v-model="form.name" autocomplete="off" @keyup.enter.native="handleSubmitFolder" />
        </el-form-item>
        <el-form-item label="Tên Thư Mục" prop="name" style="display: none">
          <el-input />
        </el-form-item>
      </el-form>
    ...
    >
      ...
      <el-row>
        ...
        >
          ...
          >
            <span>
              <el-tag type="info" size="medium" style="height: 30px">{{ tag.name }}</el-tag>
            </span>
          </el-option>
        </el-select>
      </el-row>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogMoveMulti = false">Hủy</el-button>
        <el-button

          type="primary"
          @click="handleSubmitMoveMulti"
        >Chuyển tới đây</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    async handleDeleteMultiFile() {
      const dataSubmit = this.selectedList
      for (const data of dataSubmit) {
        if (data.metaType === 1) {
          try {
            await FolderAPI.delete(data._id)
          } catch (err) {
            this.$message.error(err);
          }
        } else if (data.metaType === 2) {
          try {
            await FileAPI.delete(data._id)
          } catch (err) {
            this.$message.error(err);
          }
        }
    ...
    handleDownloadFile(file) {
      ...
      if (file?.digital && file?.digital?.sr && file?.digital?.sr?.pathPdfs[0] && file?.digital?.sr?.pathPdfs[0]?.pathPdf) {
        const pathPdf = file?.digital?.sr?.pathPdfs[0]?.pathPdf
        uri = `${config.api.download}/${pathPdf}`
      } else {
        uri = `${config.api.download}/${file.path}`
      }
      const uriDL = uri.replace('#', '%23')
      this.downloadURI(uriDL, file.name)
    },
    async getTags() {
      try {
    ...
    gotoDetail(item) {
      ...
      switch (item.metaType) {
        ...
        case this.metaType.FILE:
          if (item?.digital && item?.digital?.sr && item?.digital?.sr?.pathPdfs[0] && item?.digital?.sr?.pathPdfs[0]?.pathPdf) {
            const pathPdf = item?.digital?.sr?.pathPdfs[0]?.pathPdf
            this.$router.push(`/document/file/${item._id}?pathPdf=${pathPdf}`)
          } else {
    ...
    async handleDigitizing(item) {
      const { ok } = await DigitalDocumentAPI.checkRole(item._id)
      if (ok) {
      ...
      switch (metaType) {
        case this.metaType.FILE:
          try {
            ...
            delete file._id
            delete file.__v
            await FileAPI.move(_id, {
              ...file,
              tags: file.tags.map(tag => tag._id),
              folderId: this.targetFolderMove._id ? this.targetFolderMove._id : ""
            })
            await this.loadFolder()
            this.loadingTree = false
    ...
    handleMoveMulti() {
      this.dialogMoveMulti = true
      this.targetFolderMove = {}
      this.getTreeFolder()
    },
    async handleSubmitMoveMulti() {
      const dataSubmit = this.selectedList
      for (const data of dataSubmit) {
            if (data.metaType === 1) {
              try {
                this.loadingTree = true
                const folder = await FolderAPI.getById(data._id)
                delete folder._id
                delete folder.__v
                await FolderAPI.move(data._id, {
                  ...folder,
              ...
              try {
                ...
                delete file._id
                delete file.__v
                await FileAPI.move(data._id, {
                  ...file,
                  tags: file.tags.map(tag => tag.data._id),
                  folderId: this.targetFolderMove._id
                })
                await this.loadFolder()
                this.loadingTree = false
