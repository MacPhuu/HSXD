<template>
  <div v-loading="loadingPage" class="file-page">
    ...
    <div v-else class="app-container">
      <el-row class="heading-container" type="flex" justify="space-between">
        <el-col :span="12">
          ...
          <h3 v-else style="margin: 0; padding: 0 5px; font-weight: 600; line-height: 32px">Hồ sơ pháp lý</h3>
        </el-col>
      ...
      <div v-if="!loadingPage && tableData.length" class="table-wrap">
        ...
        >
          ...
          <el-table-column
            ...
            :index="indexMethod"
          />
          <el-table-column label="Danh sách hồ sơ pháp lý" min-width="100">
    ...
    <el-dialog
      :title="formTypeFolder === 'create' ? 'Thêm mới hồ sơ pháp lý' : 'Cập nhật hồ sơ pháp lý'"
      :visible.sync="dialogFolder"
      width="40%"
      :before-close="beforeCloseDialog"
    >
      <el-form ref="formFolder" :model="form" label-position="top" :rules="ruleFormFolder">
        <el-form-item label="Tên hồ sơ pháp lý" prop="name">
  ...
  methods: {
    ...
    gotoDetail(item) {
      ...
      switch (item.metaType) {
        case this.metaType.FOLDER: {
          const path = `/category/legalRecord/child/${item._id}?folderName=${item.name}&viewType=${this.viewType}`
          const levelList = [
            {
              title: 'Hồ sơ pháp lý',
              path: '/legalRecord/task'
            },
            {
              title: item.name,
              path,
