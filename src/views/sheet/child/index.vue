...
<!--      </el-select>-->
    ...
    <file-child v-else :folder-id="id" @updateBreadcrumb="updateBreadcrumb" @selectFile="selectFile" @selectFolder="selectFolder" />
  </div>
</template>
...
export default {
  ...
  mounted() {
    const filterList = document.getElementsByClassName('filter-box')
    const length = filterList.length
    Array.prototype.forEach.call(filterList, (item, index) => {
      if (index < length - 1) {
        item.remove()
      }
    })
